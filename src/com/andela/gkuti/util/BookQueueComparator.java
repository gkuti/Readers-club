package com.andela.gkuti.util;

import com.andela.gkuti.model.Member;
import com.andela.gkuti.model.Staff;
import com.andela.gkuti.model.Student;

import java.util.Comparator;

/**
 * BookQueueComparator class implement Comparator
 */

public class BookQueueComparator implements Comparator<Member> {
    @Override
    public int compare(Member member1, Member member2) {
        if (member1 instanceof Student && member2 instanceof Staff) {
            return -1;
        }

        if (member1 instanceof Staff && member2 instanceof Student) {
            return 1;
        }

        return compareTime(member1, member2);
    }

    public int compareTime(Member member1, Member member2) {
        int priority = 0;
        if (member1 instanceof Staff && member2 instanceof Staff && compareMembersByDate(member1, member2) > 0) {
            priority = -1;
        }
        if (member1 instanceof Staff && member2 instanceof Staff && compareMembersByDate(member1, member2) < 0) {
            priority = 1;
        }
        if (member1 instanceof Student && member2 instanceof Student && compareMembersByDate(member1, member2) > 0) {
            priority = -1;
        }
        if (member1 instanceof Student && member2 instanceof Student && compareMembersByDate(member1, member2) < 0) {
            priority = 1;
        }
        return priority;
    }

    public int compareMembersByDate(Member member1, Member member2) {
        return member1.getRegistrationDate().compareTo(member2.getRegistrationDate());

    }
}
